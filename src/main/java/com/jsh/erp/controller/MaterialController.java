...
public class MaterialController {
        ...
        try {
            ...
            if (null != dataList) {
                for (MaterialVo4Unit material : dataList) {
                   ...
                   // item.put("inputDate", material.get);
     ...
     * @return
     */
    @GetMapping(value = "/findOutBySelect")
    @ApiOperation(value = "查找商品信息")
    public JSONObject findOutBySelect(@RequestParam(value = "categoryId", required = false) Long categoryId,
                                   @RequestParam(value = "q", required = false) String q,
                                   @RequestParam("mpList") String mpList,
                                   @RequestParam(value = "depotId", required = false) Long depotId,
                                   @RequestParam("page") Integer currentPage,
        ...
        try {
            ...
            List<MaterialOutVo4Unit> dataList = materialService.findBySelectOutWithBarCode(categoryId, q, (currentPage-1)*pageSize, pageSize);

            String[] mpArr = mpList.split(",");
            int total = materialService.findBySelectOutWithBarCodeCount(categoryId, q);
            object.put("total", total);
            JSONArray dataArray = new JSONArray();
            //存放数据json数组
            if (null != dataList) {
                for (MaterialOutVo4Unit material : dataList) {
                    JSONObject item = new JSONObject();
                    item.put("id", material.getMeId()); //商品扩展表的id
                    item.put("mBarCode", material.getmBarCode());
                    item.put("name", material.getName());
                    ...
                    item.put("unitPrice", material.getUnitPrice());
                    item.put("sku", material.getSku());

                    //当前库存统计，这里的计算有点复杂, 公式为当前库存量减去出库量， 难点在于如何确定出库的数据是入库的数据
                    //BigDecimal stock;
                    //stock = depotItemService.getStockByParam(depotId,material.getId(),null,null);
                    item.put("stock", material.getOperNumber());

                    item.put("operTime", DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss").format(material.getOperTime().toLocalDateTime()));
                    dataArray.add(item);
                }
            }
            object.put("rows", dataArray);
