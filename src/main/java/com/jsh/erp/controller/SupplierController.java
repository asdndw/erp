...
public class SupplierController {
            ...
            if (null != supplierList) {
                ...
                for (Supplier supplier : supplierList) {
                    ...
                    if (!customerFlag || flag) {
                        ...
                        item.put("supplier", supplier.getSupplier()); //项目名称
                        dataArray.add(item);
                    }
                }
            }
    ...
    /**
     * 查找往来单位，含供应商和项目信息-下拉框
     * @param request
     * @return
     */
    @PostMapping(value = "/findBySelect_organ")
    @ApiOperation(value = "查找往来单位，含供应商和项目信息")
        ...
        try {
            ...
            if (null != customerList) {
                ...
                for (Supplier supplier : customerList) {
                    ...
                    if (!customerFlag || flag) {
                        item.put("id", supplier.getId());
                        item.put("supplier", supplier.getSupplier() + "[项目]"); //项目名称
                        dataArray.add(item);
                    }
                }
            }
    ...
    public JSONArray findUserCustomer(@RequestParam("UBType") String type, @RequestParam("UBKeyId") String keyId,
        ...
        try {
            ...
            outer.put("key", 0);
            outer.put("value", 0);
            outer.put("title", "项目列表");
            outer.put("attributes", "项目列表");
            //存放数据json数组
            JSONArray dataArray = new JSONArray();
            if (null != dataList) {
